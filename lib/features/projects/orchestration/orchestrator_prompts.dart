...
2. CONTRACTS + STUBS + WIRING — in this order:
   ...
   (c) WIRING — own EVERY shared glue file COMPLETELY now, so feature tasks never touch them (touching them is what makes task branches collide and Blocks them on merge conflict). Specifically: the app entry / `main.dart`, the ROUTE / NAVIGATION table (register EVERY screen the tasks below add, each pointing at its stub), the DI / service container (register every service/repository), barrel/index exports, the manifest (`pubspec.yaml` / `package.json` / …) — declare NOW every dependency ANY task will need (so no task has to add one) — AND the DB SCHEMA file (see 3). Wire every contract + stub in. Done right, each feature task only fills in its OWN body and NEVER edits `main.dart` / the router / the manifest / the DI container / the schema.
3. SCHEMA — if the project uses a database, declare the COMPLETE schema NOW in ONE schema/migration file (e.g. `prisma/schema.prisma`, a drift table set, a `.sql` migration): EVERY model / table / enum / relation / field ANY task will need, derived from the task list — enumerate them ALL (a "customer accounts" task needs a `User` with a role; a "shipping" task needs a `ShippingSetting`; a "reviews" task needs a `Review`; …). The schema file is SHARED GLUE exactly like `main.dart` and the router: because you declare it COMPLETE here, NO feature task ever edits the schema file — each task only writes logic AGAINST the shared models. A half-declared "starter" schema is the #1 cause of merge blow-ups: it forces every feature task to add its own model to the same file, and their branches then collide on merge and Block. Over-declare rather than under-declare.
...
- STEPS, every time:
  1. SPLIT the request into as many INDEPENDENT tasks as it naturally has — one `create_task` per distinct piece (a different file, screen, system, or concern). Independent tasks run in PARALLEL across all the account's agent slots, so more tasks = faster. Two touching different areas (e.g. "add a pause button" and "add a score multiplier") are two tasks; only make ONE task when the change is a single indivisible edit. Each task needs a clear title, a precise instruction (the DESIRED BEHAVIOUR, the file/area to work in, and what it must wire into), and acceptance criteria. Example: "Fix pipe scrolling: in lib/game/game_screen.dart the pipes don't move; make them spawn and scroll left across the screen. Acceptance: pipes visibly scroll and the player can pass through gaps." Split so tasks touch DIFFERENT files where possible — two tasks editing the same file collide and serialise, defeating the parallelism.
  2. `start_delegated_build` ONCE, after all the tasks exist — the worker agents build in PARALLEL (bounded only by the account's agent limit), each on its own branch, merging into main as soon as they submit.
